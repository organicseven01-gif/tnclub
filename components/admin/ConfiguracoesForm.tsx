    ...
    <form action={formAction} className="space-y-6">
      ...
      <Card padding="lg" className="space-y-5">
        <SectionTitle
          ...
          subtitle="Quantidade de serviços realizados para o cliente alcançar cada nível. Ao salvar, todos os clientes são reclassificados automaticamente."
        />

        <div className="flex items-center gap-3 rounded-2xl bg-surface px-4 py-3">
          <Award size={18} className="shrink-0 text-brand-dark" strokeWidth={1.75} />
          <p className="text-sm text-ink/60">
            <strong className="text-ink">{TIER_LABELS.bronze}</strong> é o nível inicial de todo
            cliente. O nível sobe conforme a quantidade de serviços realizados.
          </p>
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-3">
          {niveis.map((nivel) => (
            <Input
              key={nivel.name}
              label={`${nivel.label} — a partir de (serviços)`}
              name={nivel.name}
              type="number"
              min={1}
